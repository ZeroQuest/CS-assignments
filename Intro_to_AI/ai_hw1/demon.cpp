...
void Demon::move(Map& map, Actman& actman)
{
  ...
  if(map.getCell(newRow, newCol) == static_cast<char>(Sprites::actman))
  {
    ...
    std::cout<<"\t Demon Killed Actman!"<<std::endl;
    setActed(true);
  }
  ...
  {    
    map.setSpriteAtCell(static_cast<char>(Sprites::space), getPosition().row, getPosition().column);
    setPosition(newRow, newCol);
    map.setSpriteAtCell(static_cast<char>(Sprites::demon), getPosition().row, getPosition().column);
...
void Demon::onDeath(Map& map, Actman& actman)
{
...
void Demon::canMove(Map& map, Actman& actman)
{
