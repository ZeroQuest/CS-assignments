...
std::tuple<std::string, bool> boundedDepthFirstSearch(GameState& state, unsigned int depthLimit)
{
  ...
  {
    std::string initialAction(1, *it);
    frontier.push(initialAction);
    visited[initialAction] = state;
  }

  while(!frontier.empty())
  {
    std::string currentAction = frontier.top();
    frontier.pop();

    GameState currentState = visited[currentAction];

    for(auto it = currentAction.begin(); it != currentAction.end(); ++it)
    {
    ...
    if(currentState.isActmanAlive && (currentState.victory || currentState.stepNum >= 10))
    {
      return {currentAction, true};
    }

    if(currentAction.length() < depthLimit)
    {
      for(auto it = inputList.begin(); it != inputList.end(); ++it)
      {
        std::string nextAction = currentAction + *it;
        frontier.push(nextAction);
        visited[nextAction] = currentState;
      }
    }
  }
