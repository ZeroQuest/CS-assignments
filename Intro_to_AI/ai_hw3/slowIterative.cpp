...
std::tuple<std::string, bool> boundedDepthFirstSearch(GameState& state, unsigned int depthLimit)
{
  ...
  while(!frontier.empty())
  {
    ...
    if(!(shootSymCount > 1))
    {
      ...
      std::cout << "\t\tMove " << current << std::endl << std::endl;;
      
      if(current.length() == depthLimit)
      {
        for(auto it = current.begin(); it != current.end(); ++it)
        {
          if(validMove(*it, currentState.actman_row, currentState.actman_col, currentState))
            transition(currentState, *it);
          else
            break;
        }
        ...
        gameBoard = writeToString(nextState);

        //std::cout << "\t\tAfter transition!" << std::endl;
        //std::cout << "Action List: " << nextState.actionList << std::endl;
