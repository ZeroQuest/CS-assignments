...
struct Process
{
    Process() : id(999999), arrivalTime(-1), doneTime(-1), reqProcessorTime(0), processorTime(0), startTime(0), endTime(0), remainingTime(0), totalWaitTime(0), state(newArrival) {}
	...
	long startTime;		// Timestep when the process first becomes ready	
	long endTime;		// Timestep when the process completes
	long remainingTime;	// Remaining processing time left.
	long totalWaitTime; // Total time the process spends ready, but not executing
    ...
    void printProcess()
    {
		...
		//Final project Additions
		cout << setw(5) << startTime << " |";
		cout << setw(3) << endTime << " |";
		cout << setw(3) << remainingTime << " |";
		cout << setw(3) << totalWaitTime << " |";
